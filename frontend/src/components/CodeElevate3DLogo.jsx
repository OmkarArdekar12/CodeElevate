...
const useLogoTiltControls = (groupRef, meshRef) => {
  ...
  useFrame(() => {
    ...
    if (!isInteracting.current) autoRotation.current += 0.003;
  ...
  return {
    ...
    onPointerMove: (e) => {
      ...
      const x = e.movementX ?? e.touches?.[0]?.clientX - (last.current.x ?? 0);
      const y = e.movementY ?? e.touches?.[0]?.clientY - (last.current.y ?? 0);
      ...
      velocity.current.y += THREE.MathUtils.clamp(x * 0.004, -0.2, 0.2);
      velocity.current.x += THREE.MathUtils.clamp(y * 0.004, -0.2, 0.2);
    },
  };
};
...
/* =============== MAIN ================= */
const CodeElevate3DLogo = ({ cameraPosition, showGold }) => {
  return (
